...
def gen_clean_file() -> None:
    ...
    if 0:
        df3 = df[df["text_title.ko"].notna()].reset_index(drop=True)
        utils.write_str(
            utils.TEMP_TXT, "\n".join(df3["text_title.ko"].sample(100).to_list())
        )
        s1 = df3["text_title.ko"].sample(1).iloc[0]
        pat.sub("", s1)
    df["text_title.ko"] = df["text_title.ko"].parallel_apply(
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.corpus.drri.clean
            typer.run(main)
