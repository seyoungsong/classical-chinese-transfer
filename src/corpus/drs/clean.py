...
def gen_clean_file() -> None:
    ...
    if 0:
        df3 = df[df["meta.data_title.ko"].notna()].reset_index(drop=True)
        utils.write_str(
            utils.TEMP_TXT, "\n".join(df3["meta.data_title.ko"].sample(500).to_list())
        )
        s1 = df3["meta.data_title.ko"].sample(1).iloc[0]
        pat.sub("", s1)
    df["meta.data_title.ko"] = df["meta.data_title.ko"].parallel_apply(
