...
def gen_task_file() -> None:
    ...
    # add: url
    # https://db.itkc.or.kr/dir/pop/mokcha?dataId=ITKC_IT_U0
...
def main() -> None:
    # crawl index files
    gen_task_file()  # 4.8K, 4ee96fa0
    gen_dl_dir()  # 5 sec
    logger.debug(utils.folder_size(sroot.LV1_DL_DIR))  # 33.6M
    check_dl_dir()
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.drri_ko.lv1_dl
            typer.run(main)
