...
def parse1(x1: dict[Any, Any]) -> list[dict[str, Any]]:  # noqa: C901
    ...
    _url_web = f"""https://db.itkc.or.kr/dir/item?itemId=IT#/dir/node?dataId={_data_id}"""  # fmt: off
...
def main() -> None:
    ...
    if 0:
        cmd_unzip_7z()
        cmd_fix_unzip_dir()
        gen_parse_dir()  # 30sec for 15k files (m2 pro, 9 threads)
        gen_parse_concat_jsonl()  # 475.4M, a9cfd6bb
        gen_parse_file()  # 120.9M, b5281aad, 119869
    gen_parse_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.drri_ko.parse
            typer.run(main)
