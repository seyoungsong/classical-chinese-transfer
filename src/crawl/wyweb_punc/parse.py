...
def parse1(x1: dict[Any, Any]) -> list[dict[Any, Any]]:  # noqa: C901
    ...
    # check
    assert all([s.count("\t") == 1 for s in lines]), "not all lines have 1 tab"

    # parse to dataframes
    pairs_list = [x.split("\t") for x in lines if x.strip()]
    df = pd.DataFrame(pairs_list, columns=["text", "label"])
...
def gen_parse_dir() -> None:
    ...
    # get files
    fnames = list(sroot.CRAWL_DL_DIR.rglob("punc/*.tsv"))
    len(fnames)  # 3
    if 0:
...
def main() -> None:
    ...
    gen_parse_concat_jsonl()  # 98.4M, 6d4c4b05
    gen_parse_file()  # 29.6M, ab69b11c, 135134
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.wyweb_punc.parse
            typer.run(main)
