...
def gen_task_file() -> None:
    ...
    # add: url
    # https://sjw.history.go.kr/search/ajaxExpandTree.do?treeID=SJW-F&treeLevel=3
    df["url"] = df["data_id"].apply(
        lambda x: f"https://sjw.history.go.kr/search/ajaxExpandTree.do?treeID={x}&treeLevel=3"
    )
    if 0:
...
def main() -> None:
    # crawl index files
    gen_task_file()  # 5.8K, c3ef7f52
    gen_dl_dir()  # 5 sec
    logger.debug(utils.folder_size(sroot.LV1_DL_DIR))  # 38.7M
    check_dl_dir()
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.wyweb_punc.lv1_dl
            typer.run(main)
