...
def parse1(x1: dict[Any, Any]) -> list[dict[Any, Any]]:
    ...
    tags = [t for t in tags if "view.do" in str(t).lower()]
    len(tags)
    ...
    df1["page_title"] = df1["tag"].apply(
        lambda x: utils.squeeze_whites(str(x.text).strip())
    )
    df1["fname"] = str(fname)
...
def gen_parse_concat_jsonl() -> None:
    ...
    logger.debug(len(fnames))  # 104608
...
def gen_parseA_file() -> None:
    ...
    # read
    df1 = utils.read_df(sroot.LV4_TASK_PQ)
    ...
    # save
    sroot.LV4A_PQ.parent.mkdir(exist_ok=True, parents=True)
    utils.write_df2(sroot.LV4A_PQ, df)
    logger.debug(f"len: {len(df)}")
...
def main() -> None:
    ...
    cmd_unzip_7z()
    cmd_fix_unzip_dir()
    gen_parse_dir()  # 4 min, 104608
    gen_parse_concat_jsonl()  # 1.3G, e664f8fa
    gen_parse_file()
    gen_parseA_file()  # 415.2M, 84ab3183, 1787008
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.wyweb_punc.lv4_parse
            typer.run(main)
