...
def cmd_unzip_7z() -> None:
    sroot.LV2_DL_DIR.mkdir(exist_ok=True, parents=True)
    free_space = humanize.naturalsize(
        shutil.disk_usage(sroot.LV2_DL_DIR).free, gnu=True
    )
    cmd = f"""
    # FREE: {free_space}
    7za l {sroot.LV2_7Z}
    7za x {sroot.LV2_7Z} -o{sroot.LV2_DL_DIR} -y
    """
...
def gen_parse_concat_jsonl() -> None:
    ...
    # cmd
    cmd = f"""
    find {sroot.LV2_PARSE_DIR} -name '*.jsonl' -print0 | xargs -0 cat > {sroot.LV2_PARSE_CONCAT_JSONL}
    """.strip()
    sroot.LV2_PARSE_CONCAT_JSONL.parent.mkdir(exist_ok=True, parents=True)
    fname_sh = sroot.SCRIPT_DIR / "gen_parse_concat_jsonl.sh"
    utils.write_sh(fname_sh, cmd)

    # run
    utils.subprocess_run(f"bash {fname_sh}")
    utils.log_written(sroot.LV2_PARSE_CONCAT_JSONL)


def gen_parse_file() -> None:
    # read
    df = utils.read_df(sroot.LV2_PARSE_CONCAT_JSONL)
    df.info()
...
def main() -> None:
    ...
    gen_parse_file()
    gen_parseA_file()  # 11.6K, b56249bf, 983
