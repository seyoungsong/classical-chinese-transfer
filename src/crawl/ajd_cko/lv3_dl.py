...
def cmd_gen_7z() -> None:
    sroot.LV3_7Z.parent.mkdir(exist_ok=True, parents=True)
    free_space = humanize.naturalsize(
        shutil.disk_usage(sroot.LV3_7Z.parent).free, gnu=True
    )
    cmd = f"""
    df -h {sroot.LV3_7Z.parent} # {free_space}
    du -hd0 {sroot.LV3_DL_DIR}
    7za a {sroot.LV3_7Z} {sroot.LV3_DL_DIR}
    """
...
def run_gen_7z() -> None:
    ...
    utils.log_written(sroot.LV3_7Z)
...
def main() -> None:
    ...
    gen_dl_dir()
    logger.debug(utils.folder_size(sroot.LV3_DL_DIR))  # 6.4M
    check_dl_dir()
    assert_dl_dir()
    if 0:
        cmd_gen_7z()
