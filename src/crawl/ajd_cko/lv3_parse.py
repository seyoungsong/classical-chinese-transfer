...
def cmd_unzip_7z() -> None:
    sroot.LV3_DL_DIR.mkdir(exist_ok=True, parents=True)
    free_space = humanize.naturalsize(
        shutil.disk_usage(sroot.LV3_DL_DIR).free, gnu=True
    )
    cmd = f"""
    # FREE: {free_space}
    7za l {sroot.LV3_7Z}
    7za x {sroot.LV3_7Z} -o{sroot.LV3_DL_DIR} -y
    """
...
def gen_parse_dir() -> None:
    ...
    if not Path(x1["fname"]).parent.is_dir():
        ds = ds.map(
            lambda x1: {"fname": str(sroot.LV3_DL_DIR / Path(x1["fname"]).name)},
            num_proc=num_proc,
        )
    ...
    ds = ds.map(
        lambda x1: {
            "fname2": str(
                sroot.LV3_PARSE_DIR / Path(x1["fname"]).with_suffix(".jsonl").name
            )
        },
        num_proc=num_proc,
    )
...
def gen_parse_concat_jsonl() -> None:
    # check
    fnames = sorted(sroot.LV3_PARSE_DIR.rglob("*.jsonl"))
    logger.debug(len(fnames))  # 33216

    # cmd
    cmd = f"""
    find {sroot.LV3_PARSE_DIR} -name '*.jsonl' -print0 | xargs -0 cat > {sroot.LV3_PARSE_CONCAT_JSONL}
    """.strip()
    sroot.LV3_PARSE_CONCAT_JSONL.parent.mkdir(exist_ok=True, parents=True)
    fname_sh = sroot.SCRIPT_DIR / "gen_parse_concat_jsonl.sh"
    utils.write_sh(fname_sh, cmd)

    # run
    utils.subprocess_run(f"bash {fname_sh}")
    utils.log_written(sroot.LV3_PARSE_CONCAT_JSONL)


def gen_parse_file() -> None:
    # read
    df = utils.read_df(sroot.LV3_PARSE_CONCAT_JSONL)
    df.info()
    df.sample(1).iloc[0].to_dict()

    # save
    sroot.LV3_PQ.parent.mkdir(exist_ok=True, parents=True)
    utils.write_df2(sroot.LV3_PQ, df)
...
def gen_parseA_file() -> None:
    ...
    df1.drop(columns=["data_id", "row_idx", "page_title"], inplace=True)
...
def main() -> None:
    ...
    if 0:
        cmd_unzip_7z()
        cmd_fix_unzip_dir()
        utils.reset_dir(sroot.LV3_PARSE_DIR)
    gen_parse_dir()
    gen_parse_concat_jsonl()
    gen_parse_file()
    gen_parseA_file()  # 92.7K, 5a55959c, 19251
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_cko.lv2_parse
            typer.run(main)
