...
def main() -> None:
    ...
    if 0:
        ...
        gen_dl_dir()
        logger.debug(utils.folder_size(sroot.CRAWL_DL_DIR))  # 218.8M
        check_dl_dir()
        assert_dl_dir()
        cmd_gen_7z()
        run_gen_7z()  # 17.0M, 2c5713ea
    try:
        # tmux new -s crawl_dl
        # conda run --no-capture-output -n mmm python -m src.crawl.ajd_cko.crawl_dl
        # tmux attach-session -t crawl_dl
        gen_dl_dir()  # 3 min for 19392 files
    finally:
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_cko.crawl_dl
            typer.run(main)
