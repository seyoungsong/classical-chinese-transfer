...
def gen_task_file() -> None:
    ...
    # add: url
    df["temp1"] = df["data_id"].apply(lambda x: str(x).split("|")[0])
    df["temp2"] = df["data_id"].apply(lambda x: str(x).split("|")[1])
    df["url"] = df.apply(
        lambda x: f"http://esillok.history.go.kr/record/getDayListAjax.do?lang=ko&searchYearMonth={x['temp1']}&searchMonthType={x['temp2']}",
        axis=1,
    )
...
def main() -> None:
    ...
    gen_task_file()
    gen_dl_dir()
    logger.debug(utils.folder_size(sroot.LV2_DL_DIR))  # 977.4K
    check_dl_dir()
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_en.lv2_dl
            typer.run(main)
