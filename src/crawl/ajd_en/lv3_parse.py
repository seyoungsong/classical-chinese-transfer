...
def parse1(x1: dict[str, Any]) -> dict[Any, Any]:
    ...
    tags = [t for t in tags if "fnViewDetailAjax" in str(t)]
    len(tags)
    ...
    df1["data_id"] = df1["tag"].apply(lambda x: pattern.search(x.attrs["onclick"]).group(0))  # type: ignore
...
def main() -> None:
    # parse index files
    gen_lv3_file()
    gen_lv3a_file()  # 64.7K, 4311368b, 15291
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_en.lv3_parse
            typer.run(main)
