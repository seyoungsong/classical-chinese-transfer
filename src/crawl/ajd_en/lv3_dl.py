...
def gen_task_file() -> None:
    ...
    df["url"] = df["data_id"].apply(
        lambda x: f"http://esillok.history.go.kr/record/getDetailViewAjax.do?id={x}_001&sillokViewType=EngKor&lang=ko"
    )
    if 0:
...
def main() -> None:
    ...
    logger.debug(utils.folder_size(sroot.LV3_DL_DIR))  # 44.4M
    check_dl_dir()  # 11
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_en.lv3_dl
            typer.run(main)
