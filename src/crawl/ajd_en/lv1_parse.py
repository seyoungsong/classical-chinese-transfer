...
def parse1(fname1: str | Path) -> pd.DataFrame:
    ...
    tags = [t for t in tags if "fnChangeMonth" in str(t)]
    len(tags)
    if 0:
    ...
    # parse
    pat = re.compile(r"(\d+-\d+)[^\dL]+(L\d+)")
    if 0:
        t = df1["tag"].sample(1).iloc[0]
        pat.search(t.attrs["onclick"]).groups()
    df1["data_id"] = df1["tag"].apply(lambda x: "|".join(pat.search(x.attrs["onclick"]).groups()))  # type: ignore
...
def main() -> None:
    # parse index files
    gen_lv1_file()
    gen_lv1a_file()  # 5.1K, ad42d214, 235
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_en.lv1_parse
            typer.run(main)
