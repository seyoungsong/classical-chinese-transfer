...
def gen_url(x1: dict) -> str:  # type: ignore
    ...
    # convert
    id1, id2 = str(x1["data_id"]).split("|")
    url = f"http://mdb.cyberseodang.or.kr/mobile2/bookList/bookContentInside.do?bnCode={id1}&titleId={id2}"
    return url
...
def gen_url2(x1: dict) -> str:  # type: ignore
    ...
    # convert
    id1, id2 = str(x1["data_id"]).split("|")
    url2 = f"http://db.cyberseodang.or.kr/front/alphaList/BookMain.do?bnCode={id1}&titleId={id2}&compare=false"
    return url2
...
def gen_dl_dir(pool: int) -> None:
    ...
    logger.success(f"DONE: {sroot.LV2_DL_DIR}")
...
def cmd_gen_7z() -> None:
    sroot.LV2_7Z.parent.mkdir(exist_ok=True, parents=True)
    free_space = humanize.naturalsize(
        shutil.disk_usage(sroot.LV2_7Z.parent).free, gnu=True
    )
    cmd = f"""
    df -h {sroot.LV2_7Z.parent} # {free_space}
    du -hd0 {sroot.LV2_DL_DIR}
    7za a {sroot.LV2_7Z} {sroot.LV2_DL_DIR}
    """
...
def run_gen_7z() -> None:
    ...
    utils.log_written(sroot.LV2_7Z)


def main() -> None:
    # crawl
    if 0:
        utils.open_code(sroot.LV2_DL_DIR)
        utils.reset_dir(sroot.LV2_DL_DIR)
    try:
        gen_task_file()
        gen_dl_dir(pool=8)
        logger.debug(utils.folder_size(sroot.LV2_DL_DIR))
        check_dl_dir()
        assert_dl_dir()
        cmd_gen_7z()
        run_gen_7z()
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # tmux new -s crawl
            # conda run --no-capture-output -n mmm python -m src.crawl.ocdb_cc_ko.lv2_dl
