...
def gen_book() -> None:
    ...
    # use parent_id
    df1["parent_id"] = df1["data_id"].apply(lambda x: str(x).rsplit("_", maxsplit=1)[0])
    df1.sample(1).iloc[0].to_dict()
    df1["data_id"] = df1["parent_id"]
    df1.drop(columns="parent_id", inplace=True)
    df1.drop_duplicates(subset="data_id", inplace=True)
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.drri_link.book
            typer.run(main)
