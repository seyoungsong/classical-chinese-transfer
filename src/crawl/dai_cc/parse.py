...
def parse1(x1: dict[Any, Any]) -> list[dict[Any, Any]]:  # noqa: C901
    ...
    # read
    fname = Path(x1["fname"])
    txt1 = fname.read_text(encoding="utf-8").strip()
    d1 = {"text": txt1, "x1": x1}
...
def gen_parse_dir() -> None:
    ...
    # get files
    fnames = list(sroot.CRAWL_DL_DIR.rglob("*.txt"))
    fnames = sorted([p for p in fnames if p.is_file()])
    len(fnames)  # 15694
    if 0:
    ...
    if 0:
        ds.map(gen_parse_jsonl1, batched=False, load_from_cache_file=False, num_proc=1)
    for x1 in tqdm(ds):
        gen_parse_jsonl1(x1)
...
def main() -> None:
    ...
    tqdm.pandas()
    cmd_fix_unzip_dir()
    gen_parse_dir()  # 60 sec for 15694 files
    gen_parse_concat_jsonl()  # 4.8G, 0cb37aae
    gen_parse_file()  # 2.5G, dcf34458, 15694
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.dai_cc.parse
            typer.run(main)
