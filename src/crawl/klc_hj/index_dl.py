...
def gen_task_file() -> None:
    ...
    # check
    df["extra_type"] = "none"
    df.loc[df["extra"].apply(lambda x: "번역문" in x), "extra_type"] = "tran"
    df.loc[df["extra"].apply(lambda x: "교감" in x), "extra_type"] = "punc"
    df.loc[df["extra"].apply(lambda x: "번역문" in x and "교감" in x), "extra_type"] = (
        "both"
    )
    ...
    df["url"] = df["data_id"].apply(
        lambda x: f"https://db.itkc.or.kr/dir/treeAjax?dataId={x}"
    )
    if 0:
...
def main() -> None:
    # crawl index files
    gen_task_file()  # 58.4K, ad16f069
    gen_dl_dir()  # 5 sec
    logger.debug(utils.folder_size(sroot.INDEX_DL_DIR))  # 6.2M
    check_dl_dir()
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.klc_hj.index_dl
            typer.run(main)
