...
def main() -> None:
    # crawl index files
    gen_task_file()  # 349.3K, 57ff8bb5
    gen_dl_dir()  # 1 min, 17k

    logger.debug(utils.folder_size(sroot.LV2_DL_DIR))  # 12.2M
    check_dl_dir()  # 자료가 존재하지 않습니다. 12개
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.klc_hj.lv2_dl
            typer.run(main)
