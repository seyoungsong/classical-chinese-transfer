...
def gen_parse_dir() -> None:
    ...
    # get files
    fnames = list(sroot.CRAWL_DL_DIR.rglob("glner/*.jsonl"))
    len(fnames)  # 3
    if 0:
...
def main() -> None:
    ...
    cmd_fix_unzip_dir()
    gen_parse_dir()
    gen_parse_concat_jsonl()  # 22.5M, c26885c7
    gen_parse_file()  # 8.1M, 10ddb97d, 18762
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.wyweb_ner.parse
            typer.run(main)
