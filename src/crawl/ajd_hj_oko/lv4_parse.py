...
def parse1(x1: dict[Any, Any]) -> list[dict[Any, Any]]:
    ...
    # tags
    tags = list(soup.select("li a"))  # 이전, 다음 포함. 나중에 중복 제거 필요
    tags = [t for t in tags if "searchView" in str(t)]
    len(tags)
...
def main() -> None:
    ...
    cmd_unzip_7z()
    cmd_fix_unzip_dir()
    gen_parse_dir()  # 4 min on 39 threads, 158920
    gen_parse_concat_jsonl()  # 203.9M, b0de58cb
    gen_parse_file()
    gen_parseA_file()  # 12.8M, bf7c357a, 413466
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_hj_oko.lv4_parse
            typer.run(main)
