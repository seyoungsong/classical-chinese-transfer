...
def gen_task_file() -> None:
    ...
    df["url"] = df["data_id"].apply(
        lambda x: f"https://sillok.history.go.kr/search/ajaxExpandTree.do?id={x}&level=2&treeType=KING"
    )
    if 0:
...
def main() -> None:
    # crawl index files
    gen_task_file()  # 18.3K, 1ac011b6
    gen_dl_dir()  # 10 sec, 743
    logger.debug(utils.folder_size(sroot.LV2_DL_DIR))  # 3.8M
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_hj_oko.lv2_dl
            typer.run(main)
