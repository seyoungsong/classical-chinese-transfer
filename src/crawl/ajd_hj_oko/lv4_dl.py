...
def gen_task_file() -> None:
    ...
    df["url"] = df["data_id"].apply(
        lambda x: f"https://sillok.history.go.kr/search/inspectionDayList.do?id={x}"
    )
    if 0:
...
def main() -> None:
    # crawl index files
    gen_task_file()  # 1.9M, c07fee87, 158920
    gen_dl_dir()  # 40 min

    logger.debug(utils.folder_size(sroot.LV4_DL_DIR))  # 5.4G
    check_dl_dir()
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_hj_oko.lv4_dl
            typer.run(main)
