...
def gen_task_file() -> None:
    ...
    df["url"] = df["data_id"].apply(
        lambda x: f"https://sillok.history.go.kr/search/ajaxExpandTree.do?id={x}&level=1&treeType=KING"
    )
    if 0:
...
def main() -> None:
    # crawl index files
    gen_task_file()  # 6.6K, 39ba4bdf
    gen_dl_dir()  # 1 sec
    logger.debug(utils.folder_size(sroot.LV1_DL_DIR))  # 312.4K
    check_dl_dir()
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_hj_oko.lv1_dl
            typer.run(main)
