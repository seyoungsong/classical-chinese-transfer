...
def main() -> None:
    ...
    if 0:
        cmd_unzip_7z()
        cmd_fix_unzip_dir()
    gen_parse_dir()  # 20min for 413k files (m2 pro, 9 threads)
    gen_parse_concat_jsonl()  # 3.0G
    gen_parse_file()  # 971.2M, 450cd703, 826647
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.ajd_hj_oko.parse
            typer.run(main)
