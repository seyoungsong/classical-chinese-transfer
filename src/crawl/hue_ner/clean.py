...
def gen_clean_file() -> None:
    ...
    # convert
    df["split"] = df["x1.fname"].apply(lambda x: Path(x).stem.split("_")[-1])
    ...
    df["meta.king_idx"].fillna(-1, inplace=True)
    df["meta.king_idx"] = df["meta.king_idx"].astype(int)
    df["meta.king_idx"].value_counts()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.hue_ner.clean
            typer.run(main)
