...
def gen_task_file() -> None:
    ...
    # check
    df["extra_type"] = "none"
    df.loc[df["extra"].apply(lambda x: "원문" in x), "extra_type"] = "orig"
    df.loc[df["extra"].apply(lambda x: "교점" in x), "extra_type"] = "punc"
    df.loc[df["extra"].apply(lambda x: "원문" in x and "교점" in x), "extra_type"] = (
        "both"
    )
    df["extra_type"].value_counts()

    # drop: '이미지-번역문'만 있는 데이터는 우리의 관심 대상이 아님
    idx = df["extra_type"] == "none"
    df[idx].sample(1).iloc[0].to_dict()
    df = df[~idx].reset_index(drop=True)

    # add: url
    df["url"] = df["data_id"].apply(
        lambda x: f"https://db.itkc.or.kr/dir/pop/mokcha?dataId={x}"
    )
    if 0:
    ...
    df["fname"] = df["temp_id"].apply(lambda x: str(sroot.INDEX_DL_DIR / f"{x}.html"))

    # save
    sroot.INDEX_TASK_PQ.parent.mkdir(exist_ok=True, parents=True)
    utils.write_df2(sroot.INDEX_TASK_PQ, df)
...
def gen_dl_dir() -> None:
    # read
    ds = utils.read_ds(sroot.INDEX_TASK_PQ)
...
def check_dl_dir() -> None:
    # read
    fnames = list(sroot.INDEX_DL_DIR.rglob("*.html"))
...
def main() -> None:
    # build index of ids by crawling mokcha
    gen_task_file()  # 142.7K, db5fbca9
    gen_dl_dir()  # 5 sec
    logger.debug(utils.folder_size(sroot.INDEX_DL_DIR))  # 41.1M
    check_dl_dir()
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.klc_hj_ko.index_dl
            typer.run(main)
