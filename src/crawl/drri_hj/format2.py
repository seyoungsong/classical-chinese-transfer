...
def gen_format2_file() -> None:  # noqa: C901
    ...
    # text_xml to text
    if 0:
        idx = df["text_body_xml"].apply(lambda x: "font-size" in str(x))
        idx = df["text_body_xml"].apply(lambda x: "</" in str(x))
        idx.sum()
        df1 = df[idx].reset_index(drop=True)
        len(df1)
        #
        x1 = df1.sample(1).iloc[0].to_dict()
        s_xml = x1["text_body_xml"]
        etool.xml2items(s_xml=s_xml)
        etool.xml2plaintext(s_xml=s_xml)
        utils.open_url(x1["meta.url"])
        etool.names_in_xml(s_xml=s_xml)
        #
        temp1 = df1["text_body_xml"].progress_apply(etool.names_in_xml)
        logger.debug(sorted(set(sum(temp1, []))))
        s1 = "\n\n".join(df1["text_body_xml"].to_list())
        utils.write_str(utils.TEMP_TXT, s1)

    df["text_body_xml"].fillna("", inplace=True)
    df["text_body"] = df["text_body_xml"].progress_apply(etool.xml2plaintext)

    df["text_title_xml"].fillna("", inplace=True)
    df["text_title"] = df["text_title_xml"].progress_apply(etool.xml2plaintext)

    df = utils.replace_blank_to_none(df)
    if 0:
        for s_xml in tqdm(df["text_body_xml"]):
            _ = etool.xml2plaintext(s_xml=s_xml)
        x1 = df[df["text_body_xml"] == s_xml].iloc[0].to_dict()

    if 0:
        # drop text_xml if no valid NER tags
        df.drop(columns=["text_body_xml", "meta.book_extra_orig"], inplace=True)
    ...
    # drop empty rows
    df["text_body"].isna().sum()  # 158
    if 0:
        df1 = df[df["text_body"].isna()].reset_index(drop=True)
        df1.sample(1).iloc[0].to_dict()  # 좌목, 낙장(원문 훼손) 등
        df1["meta.elem_title"].value_counts()
    if 0:
        # 일성록은 body가 없는 경우가 있어서 skip
        df = df.dropna(subset=["text_body"]).reset_index(drop=True)
...
def main() -> None:
    ...
    gen_format2_file()  # 157.7M, 302272ac, 367124
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.drri_hj.format2
            typer.run(main)
