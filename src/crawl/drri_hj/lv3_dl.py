...
def gen_url(x1: dict) -> str:  # type: ignore
    ...
    # https://kyudb.snu.ac.kr/series/subitem.do?nodeCnt=123&book_cate=ILS&pid=91&py_king_nm=%EC%A0%95%EC%A1%B0&book_cd=GK12811_00&id=117&nlevel=3&label=%EC%A0%95%EC%A1%B004%20(1780%2C%20%EA%B2%BD%EC%9E%90)%2C%206%EC%9B%94&py_year=1780&py_month=06&py_yun=0&py_day=&py_century=&vol_no=&div_menu=1&item_cd=ILS&cate=1&grp_no=&upd_yn=N
    # https://kyudb.snu.ac.kr/series/subitem.do?nodeCnt=123&book_cate=ILS&pid=91&py_king_nm=정조&book_cd=GK12811_00&id=117&nlevel=3&label=정조04 (1780, 경자), 6월&py_year=1780&py_month=06&py_yun=0&py_day=&py_century=&vol_no=&div_menu=1&item_cd=ILS&cate=1&grp_no=&upd_yn=N
    url = f"https://kyudb.snu.ac.kr/series/subitem.do?nodeCnt=123&book_cate=ILS&pid=91&py_king_nm={x2['py_king_nm']}&book_cd={x2['book_cd']}&id=1000&nlevel=3&label={x2['title']}&py_year={x2['py_year']}&py_month={x2['py_month']}&py_yun={x2['py_yun']}&py_day=&py_century=&vol_no=&div_menu=1&item_cd=ILS&cate=1&grp_no=&upd_yn=N"
...
def main() -> None:
    # crawl index files
    gen_task_file()  # 45.6K, def80fa3, 1900
    gen_dl_dir()  # 1 min

    logger.debug(utils.folder_size(sroot.LV3_DL_DIR))  # 123.5M
    check_dl_dir()
    assert_dl_dir()
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.crawl.drri_hj.lv3_dl
            typer.run(main)
