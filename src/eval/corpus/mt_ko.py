...
def get_ajd_cko() -> pd.DataFrame:
    # read
    df = utils.read_df(src.crawl.ajd_cko.root.FORMAT2_PQ)
...
def get_ajd_oko() -> pd.DataFrame:
    df = utils.read_df(src.crawl.ajd_hj_oko.root.FORMAT2_PQ)
    df.sample(1).iloc[0].to_dict()
    ...
    # drop
    df.drop(columns=["lang", "text_xml"], inplace=True)
...
def get_drri_ko() -> pd.DataFrame:
    df = utils.read_df(src.crawl.drri_ko.root.FORMAT2_PQ)
    df.sample(1).iloc[0].to_dict()
    ...
    if 0:
        utils.write_str(
            utils.TEMP_TXT, "\n".join(df["meta.elem_title"].sample(100).to_list())
        )
        s1 = df["meta.elem_title"].sample(1).iloc[0]
        pat.sub("", s1)
    df["meta.elem_title"] = df["meta.elem_title"].parallel_apply(
        lambda x: pat.sub("", x)
    )

    # flatten
    cols = ["text", "meta.elem_title"]
    df2 = df[cols].stack(dropna=False).reset_index(drop=True).to_frame(name="text")  # type: ignore
    df2.dropna(subset=["text"], inplace=True)

    # check
    assert df2["text"].isna().sum() == 0

    return df2


def get_drs_ko() -> pd.DataFrame:
...
def get_klc_ko() -> pd.DataFrame:
    df = utils.read_df(src.crawl.klc_hj_ko.root.FORMAT2_PQ)
    df.sample(1).iloc[0].to_dict()
...
def get_df_ko() -> pd.DataFrame:
    # read
    df_list = [get_ajd_cko(), get_ajd_oko(), get_drri_ko(), get_drs_ko(), get_klc_ko()]
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.eval.corpus.mt_ko
            typer.run(main)
