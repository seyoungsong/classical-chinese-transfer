...
def get_drs() -> pd.DataFrame:
    # read
    df0 = utils.read_df(src.corpus.drs.root.FILTER_PQ)
    df0.sample(1).iloc[0].to_dict()
...
def get_drri() -> pd.DataFrame:
    ...
    # hj-ko (body)
    df = df0.copy()
    rcols = {"text_body.hj": "text.hj", "text_body.ko": "text.ko"}
    df.rename(columns=rcols, inplace=True)
    df.drop(columns=["text_title.hj", "text_title.ko"], inplace=True)
    df["text.type"] = "body"
    #
    idx = df["text.hj"].notna() & df["text.ko"].notna()
    round(idx.mean() * 100, 1)  # 10.0
    df_hkb = df[idx].reset_index(drop=True)
    df_hkb.sample(1).iloc[0].to_dict()
    #
    [c for c in df_hkb.columns if not c.startswith("meta")]
    rcols = {"text.hj": "text.src", "text.ko": "text.tgt"}
    df_hkb.rename(columns=rcols, inplace=True)
    #
    df_hkb["lang.src"] = "hj"
    df_hkb["lang.tgt"] = "ko"
    df_hkb.sample(1).iloc[0].to_dict()
    ...
    # hj-ko (title)
    df = df0.copy()
    rcols = {"text_title.hj": "text.hj", "text_title.ko": "text.ko"}
    df.rename(columns=rcols, inplace=True)
    df.drop(columns=["text_body.hj", "text_body.ko"], inplace=True)
    df["text.type"] = "title"
    #
    idx = df["text.hj"].notna() & df["text.ko"].notna()
    round(idx.mean() * 100, 1)  # 14.2
    df_hkt = df[idx].reset_index(drop=True)
    df_hkt.sample(1).iloc[0].to_dict()
    #
    [c for c in df_hkt.columns if not c.startswith("meta")]
    rcols = {"text.hj": "text.src", "text.ko": "text.tgt"}
    df_hkt.rename(columns=rcols, inplace=True)
    #
    df_hkt["lang.src"] = "hj"
    df_hkt["lang.tgt"] = "ko"
    df_hkt.sample(1).iloc[0].to_dict()
...
def get_ocdb() -> pd.DataFrame:
    # read
    df = utils.read_df(src.corpus.ocdb_mt.root.FILTER_PQ)
    df.sample(1).iloc[0].to_dict()
    ...
    #
    [c for c in df_hk.columns if not c.startswith("meta")]
    rcols = {"text.cc": "text.src", "text.ko": "text.tgt"}
    df_hk.rename(columns=rcols, inplace=True)
    #
    df_hk["lang.src"] = "cc"
    df_hk["lang.tgt"] = "ko"
    ...
    # drop cols
    cols = [c for c in df.columns if "_kor" in c]
    df.drop(columns=cols, inplace=True)
    df.sample(1).iloc[0].to_dict()
...
def main() -> None:
    ...
    gen_concat_file()  # 1.2G, 79ea6275, 3641606
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.dataset.mt_eval.concat
            typer.run(main)
