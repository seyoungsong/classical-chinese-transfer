...
def gen_dataset_file() -> None:
    ...
    num_klc = 625
    num_etc = 0
    df_ajd = df[df["meta.corpus"] == "ajd"].sample(n=num_ajd, random_state=42 + 100)
    df_klc = df[df["meta.corpus"] == "klc"].sample(n=num_klc, random_state=42 + 100)
    df_etc = df[~df["meta.corpus"].isin(["ajd", "klc"])].sample(
        n=num_etc, random_state=42 + 100
    )
...
def main() -> None:
    gen_dataset_file()  # 677.8K, 05aaa83e, 1250
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.train.mt_llm.ajd_klc_cc_1_8to0.dataset
            typer.run(main)
