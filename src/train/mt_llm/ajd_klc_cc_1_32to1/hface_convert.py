...
import src.train.mt_llm.ajd_klc_cc_1_32to1.root as sroot
from src import utils


def check_model_file() -> None:
    # list
    lora_dir = sroot.HFACE_LORA_DIR
    train_dir = sroot.HFACE_TRAIN_DIR
    if 0:
        utils.open_code(lora_dir)
        utils.open_code(train_dir)
    fnames = [p for p in lora_dir.rglob("*") if p.is_file()]
...
def gen_model_dir() -> None:
    ...
    # files
    fnames = [p for p in lora_dir.glob("*") if p.is_file()]
...
def main() -> None:
    check_model_file()  # 152.6M, 39bcde30
    gen_model_dir()  # 155.3M, FOLDER
    if 0:
...
if __name__ == "__main__":
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.train.mt_llm.ajd_klc_cc_1_32to1.hface_convert
            typer.run(main)
