...
import src.dataset.ner1.root
import src.train.ner1.ajd_klc.root as sroot
from src import utils


def gen_dataset_file() -> None:
    # read file
    df = utils.read_df(src.dataset.ner1.root.FORMAT_PQ)
    ...
    # filter: only ajd+klc & plain split
    logger.debug(df["meta.corpus"].value_counts())
    idx = df["meta.corpus"].isin(["ajd", "klc"]) & df["split"].isin(
        ["train", "valid", "test"]
    )
    idx.mean() * 100
...
def main() -> None:
    gen_dataset_file()  # 295.1M, c1d79a62, 380072
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.train.ner1.ajd_klc.dataset
            typer.run(main)
