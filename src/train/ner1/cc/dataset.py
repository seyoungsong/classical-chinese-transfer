...
import src.train.ner1.cc.root as sroot
...
def gen_dataset_file() -> None:
    ...
    # filter: only cc & plain split
    logger.debug(df["meta.corpus"].value_counts())
    idx = df["meta.corpus"].isin(["wyweb_ner"]) & df["split"].isin(
        ["train", "valid", "test"]
    )
    idx.mean() * 100
...
def main() -> None:
    gen_dataset_file()  # 7.5M, a0a0e068, 18719
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "ps1"):
        ...
        with logger.catch(onerror=lambda _: sys.exit(1)):
            # python -m src.train.ner1.cc.dataset
            typer.run(main)
